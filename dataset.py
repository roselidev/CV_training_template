...
def _setup_size(size, center, error_msg):
    """crop size calculator from centerpoint(x,y) --> (x,y,w,h)
    
    Args:
        size (sequence or int): Desired output size (w, h)
        center (tuple or int): Desired center point (cx, cy)
    Return:
        x1,y1,w,h
    """
...
class CustomTransform():
    ...
    def get(self,):
        tf = transforms.Compose([
            Crop(self.img_resize, self.center),
